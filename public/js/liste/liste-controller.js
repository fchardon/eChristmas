...
controllers.controller('listCtrl', function($scope, $http) {
	...
	$scope.validerCadeau = function (cadeau, username) {
		if(cadeau.titre != '') {
			 cadeau.done  = true;
			 if(cadeau.newc) {
				 creerNouveauCadeau($scope.prenomTermine, cadeau, username, $http);
			 } else {
	...
	$scope.cancelCadeau = function(cadeau) {
		console.log(cadeau.titre+" "+cadeau.newc+" "+$scope.oldTitre);
		if(cadeau.newc) {
	...
	$scope.addCadeau = function(username) {
		...
		if(canAddCadeau) {
			$scope.cadeaux.push({titre:'', done:false, newc : true});
		}
	};
	...
	$scope.addTitrePanel = function() {
		/*if($scope.signUpForm.titre.$dirty) {
			$scope.titreCadeau = $scope.titre;
			$scope.afficherTitreCadeau = true;
		} else {
...
controllers.controller('personneCtrl', function($scope, $http) {
	 ...
	 $scope.addPersonne = function() {
			$scope.personnes.push({prenom:'', done:false, newc : true});
		};
...
function creerNouveauCadeau(prenom, cadeau, user, $http)  {
	console.log("Ajouter Cadeau "+cadeau.titre+" a "+prenom+" lien:"+cadeau.lien); 
	var dataObject = {
	          titre : cadeau.titre
	          ,prenom  : prenom
	          ,lien : cadeau.lien
